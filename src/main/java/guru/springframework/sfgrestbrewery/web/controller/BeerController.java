...
public class BeerController {
    ...
    public ResponseEntity<Void> saveNewBeer(@RequestBody @Validated BeerDto beerDto){
        ...
        AtomicInteger savedBeerId = new AtomicInteger();

        beerService.saveNewBeer(beerDto)
                .subscribe(savedBeerDto -> savedBeerId.set(savedBeerDto.getId()));

        return ResponseEntity
                .created(UriComponentsBuilder
                        .fromHttpUrl("http://api.springframework.guru/api/v1/beer/" + savedBeerId.get())
                        .build().toUri())
                .build();
    }
