...
public class BeerHandlerV2 {
    ...
    public Mono<ServerResponse> updateBeer(ServerRequest request){
        return request.bodyToMono(BeerDto.class).doOnNext(this::validate)
                ...
                .flatMap(updatedBeerDto -> {
                    if(updatedBeerDto.getId() != null) {
                        log.debug("Updated Beer Id: {}", updatedBeerDto.getId());
    ...
    public Mono<ServerResponse> getBeerByUpc(ServerRequest serverRequest) {
        String upc = String.valueOf(serverRequest.pathVariable("upc"));
